...
코루틴을 이용한 비동기 처리 
● 서버나 DB에서 요청한 데이터와 UI의 불일치 현상이 자주 발생했습니다.
그래서 RxJava와 Coroutine을 사용하여 
Room DB에 값을 수정하고 GET 요청을 하는데 변경되지 않은 데이터가 응답되는 현상이 자주 발생했습니다 
그래서 코루틴을 이용하여 DB에 값을 수정 요청하면 완료가 되면 화면이 
DB에 데이터가 수정 요청을 보내고 수정이 완료되기 전에 데이터를 요청했기 때문에 UI와 데이터의 불일치 현상이 발생했습니다. 
